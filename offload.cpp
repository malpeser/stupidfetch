...
// getGpuFast - better but not
vector<string> getGpuFast() {
    vector<string> gpus;
    
    for (const auto& entry : std::filesystem::directory_iterator("/sys/class/drm/")) {
        string cardName = entry.path().filename();
        if (cardName.find("card") != 0) continue; // F8key f8ke f8kes!!!!!!!!
        
        string devicePath = entry.path().string() + "/device/";
        string vendor = readFileTrim(devicePath + "vendor");
        string device = readFileTrim(devicePath + "device");
        
        if (vendor == "0x8086") gpus.push_back("Intel GPU");
        else if (vendor == "0x10de") gpus.push_back("NVIDIA GPU");
        else if (vendor == "0x1002") gpus.push_back("AMD GPU");
    }
...
// getGpu - Get every GPU in system and pack it up in a nice vector
vector<string> getGpu(int fast) 
{
    vector<string> gpus;
    if (fast == 1) {
        gpus = getGpuFast();
    ...
    string output = exec("lspci -mm -d ::0300:");
    ...
    {
        if (line.find("VGA compatible controller") == string::npos && line.find("3D controller") == string::npos) continue;
        ...
        if (vendor == "Intel Corporation") vendor = "Intel";
        ...
        if (open != string::npos && close != string::npos && close > open) gpu_name = device.substr(open + 1, close - open - 1); else gpu_name = device;
...
string getUsageRoot()
{
    ...
    ostringstream oss;
    oss << std::fixed << std::setprecision(2)
        << usedGB << " GiB / " << totalGB << " GiB (" << GREEN << percent << "%" << RESET <<  ")";
    if (fstype != "unknown") {
        oss << " - " << fstype;
    }
    return oss.str();
